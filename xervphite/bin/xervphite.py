...
# 02110-1301, USA.
#
# xervphite: this program will read nagios host and service perfdata, and
...
config_file = '/etc/xervphite.ini'
pid_file = '/tmp/xervphite.pid'
...
spool_directory = '/var/xervphite/spool'

# xervphite log info
verbose = False
log_file = '/var/log/xervphite.log'
...
def configure(opts):
    ...
    if opts.config_file:
        ...
        if config.has_option('xervphite', 'spool_directory'): spool_directory = config.get('xervphite', 'spool_directory') 
        if config.has_option('xervphite', 'log_file'): log_file = config.get('xervphite', 'log_file') 
        if config.has_option('xervphite', 'log_level'): log_level = config.get('xervphite', 'log_level') 
        if config.has_option('xervphite', 'log_max_size'): log_max_size = config.getint('xervphite', 'log_max_size') 
        if config.has_option('xervphite', 'verbose'): verbose = config.getboolean('xervphite', 'verbose') 
        if config.has_option('xervphite', 'carbon_server'): carbon_server = config.get('xervphite', 'carbon_server') 
        if config.has_option('xervphite', 'carbon_port'): carbon_port = config.getint('xervphite', 'carbon_port') 
        if config.has_option('xervphite', 'sleep_time'): sleep_time = config.getint('xervphite', 'sleep_time') 
        if config.has_option('xervphite', 'sleep_max'): sleep_max = config.getint('xervphite', 'sleep_max') 
        if config.has_option('xervphite', 'test_mode'): test_mode = config.getboolean('xervphite', 'test_mode') 
        if config.has_option('xervphite', 'replacement_character'): replacement_character = config.get('xervphite', 'replacement_character') 
        if config.has_option('xervphite', 'pid_file'): pid_file = config.get('xervphite', 'pid_file') 
    if config.has_option('xervphite', 'graphite_prefix'): graphite_prefix = config.get('xervphite', 'graphite_prefix') 
    if config.has_option('xervphite', 'graphite_postfix'): graphite_postfix = config.get('xervphite', 'graphite_postfix') 
...
def process_nagios_perf_data(carbon_string, perf_data, time_stamp):
    """
        ...
        xervphite assumes that you have modified your plugin to always use
        the same value. If your plugin does not support this, you can use
        check_mp to force your units to be consistent. xervphite plain
        ignores the UOM.
    """
    graphite_lines = []
...
def main():
    ...
    try:
        ...
        log.info("ctrl-c pressed. Exiting xervphite.")
