...
use lib '/omd/versions/1.00/lib/perl5/lib/perl5';
...
my %conf = (
    TIMEOUT            => 15,
    CFG_DIR            => "/omd/versions/1.00/etc/pnp4nagios/",
    USE_RRDs           => 1,
    RRDPATH            => "/omd/versions/1.00/var/pnp4nagios/perfdata",
    RRDTOOL            => "/bin/true",
    RRD_STORAGE_TYPE   => "SINGLE",
    RRD_HEARTBEAT      => 8640,
    RRA_STEP           => 60,
    RRA_CFG            => "/omd/versions/1.00/etc/pnp4nagios/rra.cfg",
    STATS_DIR          => "/omd/versions/1.00/var/pnp4nagios/stats",
    LOG_FILE           => "/omd/versions/1.00/var/pnp4nagios/log/perfdata.log",
    ...
    KEY_FILE           => '/omd/versions/1.00/etc/pnp4nagios/secret.key',
    UOM2TYPE           => { 'c' => 'DERIVE', 'd' => 'DERIVE' },
    COPY_PERF_DATA_FILE => 0,
);
...
sub process_perfdata_file {
	...
	if( exists($conf{'COPY_PERF_DATA_FILE'}) && -d $conf{'COPY_PERF_DATA_FILE'} )
	{
	    use File::Copy qw(copy);
            use File::Basename;
            my $copy_dest = $conf{'COPY_PERF_DATA_FILE'} . '/' . basename($pdfile);
            print_log( "Found COPY_PERF_DATA_FILE in config. Will try to copy $pdfile to $copy_dest" );

            copy($pdfile, $copy_dest) || print_log( "Could not copy $pdfile to $copy_dest - $!" );
	}
...
sub print_help {
  ...
  Options:
    ...
    Default: /omd/versions/1.00/etc/pnp4nagios/process_perfdata.cfg
